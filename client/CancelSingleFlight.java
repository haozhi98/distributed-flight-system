...
class CancelSingleFlight{
    ...
    public static byte[] createMessage(Scanner scanner, int id)throws UnsupportedEncodingException{
        ...
        System.out.printf("Seats to cancel: %d\n", seats);
        System.out.println(Constants.CONFIRM_MSG);
        String confirm = scanner.nextLine();

        if (confirm.toLowerCase().equals("y")){
            return CancelSingleFlight.constructMessage(flightId, id, seats);
        }
        return new byte[0];
    }

    public static byte[] constructMessage(int flightId, int id, int seats)throws UnsupportedEncodingException{
        List message = new ArrayList();
        Utils.append(message, id);
        Utils.append(message, Constants.CANCEL_SINGLE_FLIGHT);
    ...
    public static void handleResponse(byte[] response, boolean debug){
        ...
        if (debug) System.out.printf("[DEBUG][CancelSingleFlight][Status = %d]\n", status);

        switch(status){
            case Constants.NAK:
                if (debug) System.out.println("[DEBUG][CancelSingleFlight][Unsuccessful response]");

                String errMsg = Utils.unmarshalMsgString(response, ptr);
                System.out.printf(Constants.ERR_MSG, errMsg);
                break;
            case Constants.ACK:
                if (debug) System.out.println("[DEBUG][CancelSingleFlight][Successful response]");
